...
optimal = []
#i+1 是核数
for i in range(len(data['unimon-LKM'])):
    total_throughput = data['unimon-LKM'][i]
...
def draw(): 
    with plt.style.context(styles): 
        fig, ax = plt.subplots()
        ax.plot(x_ticks, data['unimon-ebpf'], **line_config1["matplot_config"])
        ax.plot(x_ticks, data['unimon-LKM'], **line_config2["matplot_config"])
        ax.plot(x_ticks, data['unimon-noinv'], **line_config3["matplot_config"])
